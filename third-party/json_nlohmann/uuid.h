...
#ifdef UUID_SYSTEM_GENERATOR
#include <uuid/uuid.h> // если не найдено, использовать: sudo apt-get install uuid-dev и подключить -luuid в cmakefile
#endif
...
namespace uuids
{
   ...
   public:
      ...
      template <typename StringType>
      [[nodiscard]]  static std::optional<uuid> from_string(StringType const & in_str) noexcept
      {
         auto str = detail::to_string_view(in_str);
         bool firstDigit = true;
         size_t hasBraces = 0;
